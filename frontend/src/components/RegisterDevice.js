...
class RegisterDevice extends Component {
  ...
  async watchForChanges(txHash) {
	...
	//let deviceCreatedEvent = instance.events.DeviceCreated()
	...
	instance.events.allEvents({
	 ...
	 /*instance.events.DeviceCreated({}, { fromBlock: 0, toBlock: 'latest' }).on(
		'data', function(event) {
		console.log(event);
		let deviceCreatedEvent = event;
  ...
  async createDevice() {
    ...
    try {
	  ...
	  console.log('defaulttttttt addd:',defaultAccount);
       let result = await instance.methods.createDevice(identifierToSave2, addHexPrefix(metadataHash), addHexPrefix(firmwareHash)).send({ from: defaultAccount, gas: "4712388" })
