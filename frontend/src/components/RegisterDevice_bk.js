...
class RegisterDevice extends Component {
 ...
 divCountHandler = (deviceId) => {
  ...
  getContentForStep(step) {
	...
	console.log('getContentForStep:',identifier, metadataHash, firmwareHash, metadata, firmware,this.state)
  ...
  async createDevice() {
	...
	console.log(identifier, metadataHash, firmwareHash, address);
    ...
    try {
	  ...
	  let identifierToSave2 = this.state.web3.utils.asciiToHex('Abdul');
	  console.log('identifierToSave:',identifierToSave, identifierToSave.length);
	  console.log('identifierToSave2:',identifierToSave2, identifierToSave2.length);
	  console.log('before addedhash:',identifierToSave, metadataHash, firmwareHash)
	  console.log('after addedhash:',addHexPrefix(identifierToSave), addHexPrefix(metadataHash), addHexPrefix(firmwareHash))
	  let defaultAccount = await getDefaultAccount();
	  console.log('defaulttttttt addd:',defaultAccount);
	  
      /*await instance.methods.createDevice(addHexPrefix(identifierToSave), addHexPrefix(metadataHash), addHexPrefix(firmwareHash)).send({ from: defaultAccount, gas: "4712388" })
	  .on('receipt', async () => {
	   ...
	   .then( async () => { 
	    ...
	    let that = this;
		instance.methods.createDevice(addHexPrefix(identifierToSave), addHexPrefix(metadataHash), addHexPrefix(firmwareHash)).send({ from: defaultAccount, gas: "4712388" })
		.on('transactionHash', function(hash){
			console.log(hash);
			that.watchForChanges(hash);
			openNotificationWithIcon('info', 'Transaction sent', 'Once mined, your device will be registered.');	   
			that.setState({ loading: true })
		})
	  /* let result = await instance.methods.createDevice(addHexPrefix(identifierToSave), addHexPrefix(metadataHash), addHexPrefix(firmwareHash)).send({ from: defaultAccount, gas: "4712388" })
	   .once('receipt', (receipt) => {
		   console.log('send to blockchain')
	  this.watchForChanges(receipt);
      openNotificationWithIcon('info', 'Transaction sent', 'Once mined, your device will be registered.');	   
	 ...
	 this.watchForChanges(result.tx);
      ...
      console.log('contract errrr:',error);
      message.error(error.message);
    }
  }
