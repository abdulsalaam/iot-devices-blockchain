      ...
      switch (property) {
        case 'identifier':
          if (identifier !== identifierNew) {
            await instance.updateIdentifier(deviceId, addHexPrefix(identifierNew), { from: this.state.defaultAccount });
        ...
        case 'metadata':
          if (metadataHash !== metadataHashNew) {
            await instance.updateMetadataHash(deviceId, addHexPrefix(metadataHashNew), { from: this.state.defaultAccount });
    ...
    let identifierContent = () => {
      ...
      return (
        <div>
          ...
          {owner === this.state.defaultAccount &&
