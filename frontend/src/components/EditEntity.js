...
class EditEntity extends Component {
  ...
  async updateMyData() {
    try {
	  //let instance = await DeviceManager;
      let result = await this.state.instance.methods.ownerToEntity(this.state.defaultAccount).call();
  ...
  async saveMyData() {
    try {
      if (this.state.myDataNew !== this.state.myData) {
        let instance = await DeviceManager;
        await instance.methods.updateEntityData(this.state.myDataNew).send({ from: this.state.defaultAccount });
  ...
  render() {
    return (
      <div>
        ...
        <Card style={{ maxWidth: '500px' }} loading={this.state.loading} title={this.state.defaultAccount}>
          {this.state.showEdit ?
            <div>
